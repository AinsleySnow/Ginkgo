...
Expr* Parser::ParsePrimaryExpr()
{
    ...
    if (tok->IsIdentifier())
    {
        ...
        if (ident)
            return ident;
        if (IsBuiltin(tok->str_))
            return GetBuiltin(tok);
...
Constant* Parser::ParseInteger(const Token* tok)
{
    ...
    if (decimal)
    {
        ...
        {
        case 0:
            tag |= !(val & ~(long)INT_MAX) ? T_INT : T_LONG;
            break;
        case T_UNSIGNED:
    ...
    else
    {
        ...
        case T_UNSIGNED:
            ...
            break;
        case T_LONG:
            tag |= !(val & ~(long)LONG_MAX) ? 0 : T_UNSIGNED;
            break;
        case T_UNSIGNED | T_LONG:
            break;
        }
...
UnaryOp* Parser::ParsePrefixIncDec(const Token* tok)
{
    ...
    auto op = tok->tag_ == Token::INC ? Token::PREFIX_INC : Token::PREFIX_DEC;
...
Expr* Parser::ParseRelationalExpr()
{
    ...
    auto tok = ts_.Next();
    while (tok->tag_ == Token::LE || tok->tag_ == Token::GE || tok->tag_ == '<' || tok->tag_ == '>')
    {
...
int Parser::ParseQual()
{
    ...
    for (;;)
    {
        ...
        case Token::VOLATILE:
            ...
            break;
        case Token::ATOMIC:
            Error(tok, "do not support 'atomic'");
            break;
        default:
            ts_.PutBack();
            return qualSpec;
    ...
    int align)
{
    ...
    if (storageSpec & S_TYPEDEF)
    {
        ...
        if (ident)
        { // There is prio declaration in the same scope
...
Declaration* Parser::ParseInitDeclarator(Identifier* ident)
{
    ...
    if (!obj)
    { // Do not record function Declaration
        return nullptr;
    }
